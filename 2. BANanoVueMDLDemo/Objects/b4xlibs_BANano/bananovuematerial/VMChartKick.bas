...
Sub SetStyleSingle(prop As String, val As String) As VMChartKick
...
Sub SetRefresh(refresh As Int) As VMChartKick
	SetAttrSingle(":refresh", refresh)
	Return Me
End Sub
...
Sub SetAttrSingle(prop As String, val As Object) As VMChartKick
...
Sub ToString As String
	...
	Else
		...
		plotJSON = BANano.ToJson(plotList)
		plotJSON = plotJSON.Replace(QUOTE,"'")
		SetData(plotJSON)
	End If
	If colors.Size > 0 Then
		Dim colorJSON As String = BANano.ToJson(colors)
		colorJSON = colorJSON.Replace(QUOTE,"'")
		SetColors(colorJSON)
	End If
