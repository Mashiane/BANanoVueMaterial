...
Sub SetTitle(sTitle As String) As VMGijgoTable
	...
	spanTitle.Initialize(vue,"").SetTag("span")
...
Sub AddColumn(colName As String, colTitle As String) As VMGijgoTable
	...
	SetColumnSortable(colName, True)
	Return Me
End Sub
...
'right align columns
Sub SetColumnsAlignmentCenter(flds As List) As VMGijgoTable
...
'right align columns
Sub SetColumnsAlignmentRight(flds As List) As VMGijgoTable
...
Sub SetColumnsFormatDate(dates As List) As VMGijgoTable
...
Sub SetColumnsFormatDateTime(dates As List) As VMGijgoTable
...
'set the column type to date time for these columns
Sub SetFileSizeColumns(dates As List) As VMGijgoTable
	For Each k As String In dates
...
Sub SetColumnsFormatFileSize(dates As List) As VMGijgoTable
...
Sub SetColumnsFormatTime(dates As List) As VMGijgoTable
	SetTimeColumns(dates)
	Return Me
End Sub

Sub SetColumnsFormatMoney(dates As List) As VMGijgoTable
...
Sub SetColumnsSortable(colNames As List, b As Boolean)
	For Each col As String In colNames
		SetColumnSortable(col, b)
	Next
End Sub

Sub SetColumnsFilterable(colNames As List, b As Boolean)
	For Each col As String In colNames
		SetColumnFilterable(col, b)
	Next
End Sub

Sub SetColumnsFormat(colNames As List, f As String)
	For Each col As String In colNames
		SetColumnFormat(col, f)
	Next
End Sub
...
Sub IconRender(value As Object, record As Map, cell As BANanoObject, displayEl As BANanoObject)
	If value = Null Then
		...
		Return
	End If
	Dim sfontsize As String = record.GetDefault("iconsize","")
	Dim sfontcolor As String = record.GetDefault("iconcolor","")
...
Sub ImageRender(value As Object, record As Map, cell As BANanoObject, displayEl As BANanoObject)
	...
	If value <> Null Then
		Dim simagewidth As String = record.GetDefault("imagewidth","")
		Dim simageheight As String = record.GetDefault("imageheight","")
